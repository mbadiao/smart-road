            ...
            match self.direction {
                ...
                Direction::East => {
                   let  egale = false;
                    if dir == Direction::North && (turn == Turn::Forward || (turn == Turn::Left && self.turn == Turn::Left)) {
                        ...
                        if self.x < vx && (vx - self.x).abs() >= ( vy - self.y).abs() && vy > self.y  && (self.x > 210) {
                            any_collision = true;
                ...
                Direction::West => {
                    ...
                    if dir == Direction::North && (turn == Turn::Forward || (turn == Turn::Left && self.turn == Turn::Left)) {
                        ...
                        if self.x > vx && (self.x - vx).abs() >= ( vy - self.y).abs() && vy > self.y  && self.x < 460{
                            any_collision = true;
    ...
    pub fn update_position(&mut self, vehicle_data: &Vec<(i32, i32, Direction, Turn)>) {
        ...
        if self.is_at_intersection_start() {
            if self.turn != Turn::Right {
                ...
                //         Direction::East => {
                //             match self.collision(vehicle_data).1 .1 {
                //                 Direction::North=> stop = true,
                //                 Direction::South=> ,
                //                 Direction::West=>,
                //                 Direction::East=>,
